package com.grouptwo.isrp.dao;

import com.grouptwo.isrp.entity.IsrpUser;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import org.springframework.data.domain.Pageable;

import java.util.List;

/**
 * 用户表(IsrpUser)表数据库访问层
 *
 * @author makejava
 * @since 2022-06-19 19:08:33
 */
@Mapper
public interface IsrpUserDao {

    /**
     * 通过ID查询单条数据
     *
     * @param userId 主键
     * @return 实例对象
     */
    IsrpUser queryById(String userId);

    /**
     * 查询指定行数据
     *
     * @param isrpUser 查询条件
     * @param pageable         分页对象
     * @return 对象列表
     */
    List<IsrpUser> queryAllByLimit(IsrpUser isrpUser, @Param("pageable") Pageable pageable);

    /**
     * 统计总行数
     *
     * @param isrpUser 查询条件
     * @return 总行数
     */
    long count(IsrpUser isrpUser);

    /**
     * 新增数据
     *
     * @param isrpUser 实例对象
     * @return 影响行数
     */
    int insert(IsrpUser isrpUser);

    /**
     * 批量新增数据（MyBatis原生foreach方法）
     *
     * @param entities List<IsrpUser> 实例对象列表
     * @return 影响行数
     */
    int insertBatch(@Param("entities") List<IsrpUser> entities);

    /**
     * 批量新增或按主键更新数据（MyBatis原生foreach方法）
     *
     * @param entities List<IsrpUser> 实例对象列表
     * @return 影响行数
     * @throws org.springframework.jdbc.BadSqlGrammarException 入参是空List的时候会抛SQL语句错误的异常，请自行校验入参
     */
    int insertOrUpdateBatch(@Param("entities") List<IsrpUser> entities);

    /**
     * 修改数据
     *
     * @param isrpUser 实例对象
     * @return 影响行数
     */
    int update(IsrpUser isrpUser);

    /**
     * 通过主键删除数据
     *
     * @param userId 主键
     * @return 影响行数
     */
    int deleteById(String userId);

    IsrpUser queryByEmail(String email);



    int getuserStatusByUserId(String userId);

    int changeStatus(String userId);

    List<IsrpUser> getAlluser();
}

